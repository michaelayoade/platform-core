...
from app.modules.webhooks.models import (
    ...
    WebhookEndpointCreate, WebhookEndpointUpdate, WebhookSubscriptionCreate,
    WebhookStatus, WebhookEventType
)
...
class WebhooksService:
    ...
    async def get_endpoints_for_event(
        ...
        results = db.query(WebhookEndpoint, WebhookSubscription).join(
            WebhookSubscription, 
    ...
    async def _deliver_webhook(
        ...
        try:
            ...
            if db_delivery.success:
                logger.info(f"Webhook delivery successful: ID={db_delivery.id}, Event={event_type}, Status={response.status_code}")
            else:
                logger.warning(f"Webhook delivery failed: ID={db_delivery.id}, Event={event_type}, Status={response.status_code}")
    ...
    async def retry_failed_deliveries(db: Session) -> int:
        ...
        now = datetime.utcnow()
        failed_deliveries = db.query(WebhookDelivery).join(
            WebhookEndpoint, 
