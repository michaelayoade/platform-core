...
from app.modules.webhooks.models import (
    ...
    WebhookEndpointCreate, WebhookSubscriptionCreate,
...
def test_create_endpoint(client, db_session):
    ...
    # Check database
    db_endpoint = db_session.query(WebhookEndpoint).filter(
        WebhookEndpoint.id == data["id"]
    ).first()
...
def test_create_subscription(client, db_session):
    ...
    # Check database
    db_subscription = db_session.query(WebhookSubscription).filter(
